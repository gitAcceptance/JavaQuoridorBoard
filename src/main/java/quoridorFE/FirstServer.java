...
/*
 * The server that is for user imput!
 */
public class FirstServer {
    ...
    private ArrayList<ClientThreadb> al = new ArrayList<ClientThreadb>();
    // to display time
     ...
     *  in console
     */
    public FirstServer(String machine, int port1) {
        port = port1;
        machineName = machine;
    }
    ...
    public void start() {
        ...
        try{
            ...
            while(keepGoing) {
                ...
                if(!keepGoing)
                    break;
                ClientThreadb t = new ClientThreadb(socket);  // make a thread of it
    ...
    class ClientThreadb extends Thread {
            // String constant to pose as playername in the IAM message
            String PLAYERNAME = "fex:TESTBOT";
        ...
        // IO handlers
            ...
            public void run() {
				...
				System.out.println("Four players locked in!");
                        ...
                        System.out.println("A move is requested");
...
//????????????????????????? 
//????????????????????????? will this clutter removal effect our protocol satisfaction?
//?????????????????????????
			...
			if(message.contains("h") || message.contains("v")){
				...
				//board.movePawn(Integer.parseInt(sc[1]), Integer.parseInt(sc[2]), Integer.parseInt(sc[3]));
                        ...
                        System.out.println("Recieved: " + message);
                        System.out.println("Player was booted!");
